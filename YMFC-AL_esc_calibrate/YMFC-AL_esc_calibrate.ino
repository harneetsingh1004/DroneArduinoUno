...
void setup(){
  ...
  Serial.println("storing gyro address");
  ...
  for(int i = 0; i<1024;i++){
     byte data = eeprom_data[i];
     Serial.print(i);
     Serial.print("  : ");
     Serial.println(data,HEX);
  }
  delay(10000);
  Serial.println("setting the gyro register");
  set_gyro_registers();                                                                 //Set the specific gyro registers.
  Serial.println(" Enter into while");
  ...
  Serial.println("Leaving while loop");
  wait_for_receiver();                                                                  //Wait until the receiver is active.
  zero_timer = micros();                                                                //Set the zero_timer for the first loop.
  Serial.println("Enter in second while");
...
void set_gyro_registers(){
  ...
  if(eeprom_data[31] == 1){
    ...
    Serial.println("entering in while inside set_gyro_register");
    while(Wire.available() > 6);                                 //Wait until the 6 bytes are received
